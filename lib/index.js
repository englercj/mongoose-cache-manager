...
module.exports = function mongooseCache(mongoose, options, cb) {
    ...
    cache = cacheManager.caching(defaultOptions);
    ...
    proto.exec = function (op, callback) {
        ...
        var script =
            "local collectionData = redis.call('get', KEYS[1])\n" +
            "local queryData = redis.call('get', KEYS[2])\n" +
            "\n" +
            "if not collectionData or not queryData or cjson.decode(collectionData).lastWrite > cjson.decode(queryData).metadata.lastWrite then\n" +
            "    return nil;\n" +
            "end\n" +
            "\n" +
            "return {collectionData,queryData}";

        cache.store.client.eval(script, 2, collectionKey, queryKey, function (err, results) {

        // cache.store.client.multi().get(collectionKey).get(queryKey).exec(function (err, results) {
