...
module.exports = function mongooseCache(mongoose, options, cb) {
    ...
    proto.exec = function (op, callback) {
        ...
        async.parallel([
            cache.get.bind(cache, collectionKey),
            cache.get.bind(cache, queryKey)
        ], function (err, results) {
            //TODO: Should we treat a cache error as just a cache miss??
            if (err) return callback(err);

            var collectionData = JSON.parse(results[0]),
                queryData = JSON.parse(results[1]);
            ...
            if (!queryData || (collectionData && collectionData.lastWrite > querydata.metadata.lastWrite)) {
                ...
                proto._exec.call(self, function (err, docs) {
                    ...
                    cache.set(
                        queryKey,
                        ttl,
                        self._createCacheData(docs, collectionData && collectionData.lastWrite)
                    );
    ...
    proto._createCacheData = function (docs, lastWrite) {
